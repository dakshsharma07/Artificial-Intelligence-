...
class Perceptron(object):
    ...
    def __init__(self, inSize=1, weights=None):
        ...
        if weights is None:
            #weights of previous layers into this one, random if passed in as None͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
    ...
    def sigmoid(self, value):
        ...
        """
        """YOUR CODE"""
        return float(1 / (1 + exp(-value))) #refer to slides april 11
    ...
    def sigmoidActivation(self, inActs):
        ...
        """YOUR CODE"""
        x = [1.0] + inActs #per Ed
        return float(1 / (1 + exp(-self.getWeightedSum(x))))  #refer to slides april 11
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        """YOUR CODE"""
        return float(self.sigmoid(value) * (1 - self.sigmoid(value))) #refer to slides april 11
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        return self.sigmoidActivation(inActs) * (1 - self.sigmoidActivation(inActs)) #same as one above
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        totalModification = 0
        """YOUR CODE"""
        #taking weight formula: weight = weight + alpha * ai * delta, where ai is the modified input
        x = [1] + inActs #from ed to add bias
        i = 0
        modification = []
        while i < len(x):
            self.weights[i] += alpha * x[i] * delta #updating the weights here
            modification.append(abs(alpha * x[i] * delta)) #using this to return the total modification
            i += 1
        totalModification = sum(modification)
...
class NeuralNet(object):
    ...
    def __init__(self, layerSize):#default 3 layer, 1 percep per layer
        ...
        #build hidden layer(s)͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
        ...
        #build output layer͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
        for i in range(layerSize[-1]):
            percep = Perceptron(layerSize[-2]) # num of perceps feeding into this one
            self.outputLayer.append(percep)

        #build layers list that holds all layers in order - use this structure͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
        # to implement back propagation͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
    ...
    def feedForward(self, inActs):
        ...
        """
        """YOUR CODE"""
        x = [inActs] #list of a list that will be returned, this is the input list
        l = 0
        while l < len(self.layers): #going through all the layers (list of perceptrons) to propogate results (see ED )
            out = []
            for i in self.layers[l]:
                out.append(i.sigmoidActivation(x[-1])) #getting the outputs
            x.append(out)
            l += 1
        return x
    ...
    def backPropLearning(self, examples, alpha):
        ...
        """
        #keep track of output͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
        averageError = 0
        averageWeightChange = 0
        numWeights = 0

        for example in examples:#for each example
            #keep track of deltas to use in weight change͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
            deltas = []
            #Neural net output list͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
            allLayerOutput = self.feedForward(example[0])#"""FILL IN - neural net output list computation"""
            lastLayerOutput = allLayerOutput[-1]
            #Empty output layer delta list͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
            outDelta = []
            #iterate through all output layer neurons͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
            for outputNum in range(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[-2])#"""FILL IN"""
                error = example[1][outputNum] - lastLayerOutput[outputNum] #Target - Actual to see the error """FILL IN - error for this neuron"""
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                hiddenDelta = []
                #Iterate through all neurons in this layer͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
                for neuronNum in range(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum]) #"""FILL IN"""
                    #Need to find error again to do the delta formula = error * gprime
                    nError = 0
                    for pair in zip(nextLayer, deltas[0]):
                        nError += pair[0].weights[neuronNum + 1] * pair[1]
                    delta = gPrime * nError
            ...
            #end for each example͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
        #calculate final output͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    #Ed Comment by Reetesh for pseudo/walkthrough
    weightMod += .1 #adding to make sure loop below starts
    while weightMod > weightChangeThreshold:
        if iteration < maxItr: 
    ...
    #second half of reetesh explanation
    for i in examplesTest:
        out = nnet.feedForward(i[0])[-1] #feed forward the examples
        if all(round(o) == e for o, e in zip(out, i[1])): #check for any errors
            testCorrect += 1.0
        else:
            testError += 1.0 #increase by 1 if expected value doesn't match the value determined
    numTotal = len(examplesTest) * 1.0
    testAccuracy = testCorrect/numTotal
