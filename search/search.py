...
def depthFirstSearch(problem):
    ...
    reached = set()
    reached.add(node)
    frontier.push((node, neighbor))
    while not frontier.isEmpty():
        node, neighbor = frontier.pop()
        ...
        for child, action, _ in problem.getSuccessors(node):
            if child not in reached:
                frontier.push((child, neighbor + [action]))
    #util.raiseNotDefined()
...
def uniformCostSearch(problem):
    ...
    frontier.push((node, neighbor, 0), 0)
    while not frontier.isEmpty():
        node, neighbor, stepCost = frontier.pop()
        ...
        for child, action, cost in problem.getSuccessors(node):
            if child not in reached:
                frontier.push((child, neighbor + [action], stepCost + cost), stepCost + cost)
    #util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not frontier.isEmpty():
        ...
        for child, action, cost in problem.getSuccessors(node):
            if child not in reached:
                heuristicCost = stepCost + cost + heuristic(child, problem)
                frontier.push((child, neighbor + [action], stepCost + cost), heuristicCost)
    #util.raiseNotDefined()


# Abbreviations͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
