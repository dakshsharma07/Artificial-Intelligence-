...
class InferenceModule:
    ...
    """
    ############################################
    # Useful methods for all inference modules #͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
    ############################################
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        #raiseNotDefined()͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
    ...
    ######################################
    # Methods that need to be overridden #͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
    ######################################
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in self.allPositions:
            self.beliefs[i] = self.getObservationProb(observation, gameState.getPacmanPosition(), i, self.getJailPosition()) * self.beliefs[i]
        
        #another way to call .normalize as it was throwing an exception
        total = sum(self.beliefs.values())
        if total > 0:
            for i in self.beliefs:
                self.beliefs[i] = self.beliefs[i] / total
    ...
    def elapseTime(self, gameState):
        ...
        dist = DiscreteDistribution()
        dist = {i: 0 for i in self.allPositions}
        for i in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, i)
            for j in newPosDist.items():
                dist[j[0]] += j[1] * self.beliefs[i]
        self.beliefs = dist
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        #first part mirrors q1
        tmp = DiscreteDistribution()
        for i in range(len(self.particles)):
            tmp[self.particles[i]] += self.getObservationProb(observation, gameState.getPacmanPosition(), self.particles[i], self.getJailPosition()) 
        tmp.normalize()

        if tmp.total() == 0: #utiliing total method to find out if all particles recieved 0 weight
            self.initializeUniformly(gameState) #calling initializeUniformly if they did
        else:
            newPart = [] #creating new list
            for i in range(self.numParticles): #looping number of the particles
                newPart.append(tmp.sample()) #utilizing sample() to resample and construct new list of particles
    ...
    def getBeliefDistribution(self):
        ...
        tmp = DiscreteDistribution()
        for i in range(len(self.particles)):
            tmp[self.particles[i]] += 1
        tmp.normalize() #coverting the list into a normalized distribution
        return tmp
...
class JointParticleFilter(ParticleFilter):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # now loop through and update each entry in newParticle...͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
            "*** YOUR CODE HERE ***"
            raiseNotDefined()
...
# One JointInference module is shared globally across instances of MarginalInference͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
