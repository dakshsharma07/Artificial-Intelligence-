# valueIterationAgents.py͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
# -----------------------͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        # Write value iteration code here͏󠄂͏️͏󠄌͏󠄎͏󠄎͏︇͏󠄌
        "*** YOUR CODE HERE ***"
        for __ in range(self.iterations):
            new_vals = util.Counter() #from self.values = util.Counter(), U = U'
            max_q = 0 # set max = 0
            for state in self.mdp.getStates(): # for each state s in S do:
                if not self.mdp.isTerminal(state): #as long as s is not in end state
                    q_vals = [] 
                    for action in self.mdp.getPossibleActions(state):
                        q = self.computeQValueFromValues(state, action)
                        q_vals.append(q)
                    max_q = max(q_vals) #find max of all the q values computed
                    new_vals[state] = max_q
            self.values = new_vals # reset the values, returning U
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next, probs in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            q += probs * (reward + self.discount * self.getValue(next)) # Follow formula from bellman equations
        return q
    ...
    def computeActionFromValues(self, state):
        ...
        for action in self.mdp.getPossibleActions(state): #get all actions
            v = self.getQValue(state, action) 
            actions.append(v)
            if v == max(actions): #if value has the max action
                best = action
        if not self.mdp.getPossibleActions(state): #no legal actions
            return None
        return best # returns best action
